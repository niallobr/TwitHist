...
*	by Niall O'Brien
*
* Credit to Jon Hurlock for original Oauth.php
*
*/
...
print search_for_a_term($bearer_token, "history"); //  search for 'history'
...
* Based on
*/
    function pull_timeline($bearer_token, $screen_name, $count='15'){
    $url = "https://api.twitter.com/1.1/statuses/user_timeline.json"; // base url
    $screen_name = urlencode(trim("History_Pics")); // query term
    $formed_url ='?screen_name='.$screen_name; // fully formed url
    if($count!='15'){$formed_url = $formed_url.'&count='.$count;} // results per page - defaulted to 15
    $headers = array(
...
// lets print timeline
$bearer_token = get_bearer_token(); // get the bearer token
print pull_timeline($bearer_token, $screen_name); //  print results
invalidate_bearer_token($bearer_token); // invalidate the token
